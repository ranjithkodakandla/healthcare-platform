...
**Document version:** 4.1 · **Last updated:** 2026-07-30 · **Updated by:** Claude Code (Sonnet 5) (Session 38 — UAT feedback triage pass, 37 items)
...
- **TD-028** — Admin Console has no in-app way for an admin (during onboarding, A-04/A-05) or a hospital admin (Provider Portal) to attach/upload supporting documents to a provider application — `documents` on `ProviderApplicationDetail` is a read-only, hardcoded-per-`providerType` list of expected document *names* with per-key download stubs, not an actual upload/storage flow. UAT #6.6/#9. Needs an actual object-storage-backed upload endpoint (`/shared-services/documents`, referenced in the architecture diagram but never built) before this can be closed — scoped out of this UAT pass as non-trivial.
- **TD-029** — `apps/admin-console`'s onboarding form (`onboarding/provider/page.tsx`) uses "Apollo Hospital, Whitefield" as its legal-name input placeholder, and a Jest fixture (`admin-ops.service.unit.spec.ts`) uses `{ name: 'Apollo' }` as mock hospital data. Both are placeholder/test-only text, not persisted seed/demo data — but see DL-018 (UAT #7) for the open product question of whether real-hospital-brand references should be scrubbed from example/placeholder copy platform-wide.
- **TD-030** — `AdminOpsService.getMonitoringSnapshot`'s "Ambulance Matching"/other health cards on A-14 are computed from live counts (good), but the platform still has no dedicated alerting/paging integration — "Active incidents" is entirely derived (HIGH `PlatformIssue`s + a HIGH-ticket-count synthetic row), not a real incident-management feed. Not a bug (correctly labelled, live data), just worth flagging as the monitoring surface's known ceiling for this MVP phase.
...
| DL-018 | 2026-07-27 | UAT #7 — whether real-hospital-brand references (e.g. "Apollo") should be scrubbed from all example/placeholder/demo copy platform-wide | Open | Stakeholder (Mohammed Saad Ansari) UAT feedback flagged this as a content/legal question, not a code bug. Current instances found are placeholder text (onboarding form's legal-name input placeholder) and a Jest fixture's mock data — not persisted seed/demo data shown to real users. Needs a product/legal decision on the platform's naming policy for illustrative examples before any rename; deferred rather than guessed. See TD-029. |
| DL-019 | 2026-07-27 | UAT #10/#22 — precise meaning of "Go Live" vs. "Access Activate" (A-05), and what a defined test/usage flow for Remote Session Assist (A-08) should be | Open | Both are UX-copy/workflow questions the PRD's G4/G6 narrative sections don't spell out at the level of "what should the admin literally click and see". `ProviderOnboardingService.isPortalLive` (Phase 3) is the actual "Go Live" gate (all 5 onboarding stages `COMPLETE`); "Access Activate" isn't a distinct system state anywhere in the current code — it may be UI copy from an older prototype screen that no longer matches the shipped stage names (`APPLICATION_INTAKE → CREDENTIAL_VERIFICATION → INTEGRATION_TEST → GO_LIVE_APPROVAL → PORTAL_ACCESS_ACTIVATED`). Needs product sign-off on canonical stage-label copy, and a real defined flow (who initiates a remote session, with what tooling, against which portal) for Remote Session Assist before either can be implemented/labelled with confidence. |
| DL-020 | 2026-07-27 | UAT #23/#24 — Issue Tracking Board (A-09) workflow/state-machine policy: which Kanban column-to-column transitions are legal, and whether a change-reason should be mandatory on every status update | Open | `AdminOpsService.updateIssueStatus` currently accepts any `status` string with no transition-legality check and no reason field (unlike `SupportTicket`, which now requires reopening a closed ticket explicitly — see this session's UAT #21 fix). Needs a product decision on the board's actual allowed state machine (e.g. can a `RESOLVED` issue go straight back to `OPEN`, or must it pass through `IN_PROGRESS`?) before implementing a real guard — the PRD's G7 section describes the board narratively but doesn't enumerate transitions. |
| DL-021 | 2026-07-27 | UAT #26 — SLA Monitoring (A-10): should SLA definitions/targets be scoped per-org/per-hospital, or remain platform-wide only | Open | `AdminOpsService.getSlaAndAnalyticsSnapshot` (this session's fix corrected its zero-volume-shows-as-BREACH bug, see UAT #25 / Session 38 below) computes every metric platform-wide with no org dimension. Whether a Console Administrator should be able to view/set SLA compliance per hospital (vs. only in aggregate) is a scope decision the PRD's G7/FR-ADM-SLA-001 doesn't resolve explicitly — needs product input before building per-org filtering. |
| DL-022 | 2026-07-27 | UAT #29 — Feature Flags "Configuration" tab (A-18): should a Console Administrator be able to edit configuration values in-app, and which config keys are plan-gated vs. freely editable | Open | `AdminOpsService.listConfig` currently returns `PlatformConfig` rows read-only (grouped by `groupKey`, e.g. `sla`/`hold_expiry`) with no `PATCH`/update endpoint or UI — only Feature Flags (a separate tab) support toggling. Whether Configuration should gain write access, and what governs which rows are safe to expose for in-app editing vs. requiring a deploy, is a product/security policy decision not resolved by G15/G16's narrative PRD text. |
| DL-023 | 2026-07-27 | UAT #31 — Audit log entity/actor readability: broader UX policy beyond this session's UAT #30 actor-name fix | Open | This session resolved the narrowest form of #30 (actor uid → resolved console-user email+role when known, `admin-ops.service.ts` `searchAudit`). Left open: `entityType:entityId` in the Audit Search table (A-18) still renders as `HospitalRegistry:a1b2c3d4…` — a raw type+truncated-UUID pair — rather than e.g. the hospital's name. Resolving that generally requires a per-`entityType` label-resolution strategy (Case → caseNumber, HospitalRegistry → name, SupportTicket → ticketNumber, etc.) that wasn't in this session's scope; flagged for a follow-up pass rather than guessed at row-by-row. |
| DL-024 | 2026-07-27 | UAT #33/#34 — Providers screen (A-20): whether total bed count (not just availability) should be admin-editable, and whether providers should be grouped/filtered by the PRD's "7 categories" (Hospital/Ambulance/Blood Bank/Doctor/Pharmacy/Diagnostic/Insurer) | Open | Current admin bed-adjustment UI (`providers/page.tsx` `bumpAvailable`) only lets an admin move `availableCount` within the existing `totalCount` (a deliberate safety rail — total capacity is meant to be provider-reported, not admin-overridden, per the platform's "providers keep bed data current" trust model, PRD Part D point 2). Whether an admin override of total capacity should ever be allowed (e.g. for correcting a clearly wrong provider-entered number) is a trust/audit policy question. Separately, the Providers directory currently has no category filter/tabs at all (search-only) — whether it should adopt the PRD's provider-type taxonomy as first-class navigation is a design decision, not implemented this session. |
| DL-025 | 2026-07-27 | UAT #37 — formal role-permission matrix documentation for the four non-admin console roles (Support Agent, Provider Onboarding Specialist, Trust and Safety Analyst, Compliance Officer) | Open | The actual enforcement exists in code (`ConsoleUserService`'s `ConsoleRole` catalogue + `AdminOpsController`'s per-handler `requireSupport`/`requireOps`/`requireAdminOrCompliance` gates), but there is no single human-readable matrix mapping each of the 4 roles to exactly which A-0x screens/actions they can reach — this document's own G9 minimal-slice note and the controller's per-method guard calls are the closest things to it today, scattered across files. Documenting this was explicitly out of scope for a code-bug triage pass (no code changes implied) — logged for a follow-up documentation task, likely as a table added directly to this plan once the guard list is confirmed exhaustive against every controller method. |
...
### Session 38 — 2026-07-30
- **AI Tool Used:** Claude Code (Sonnet 5)
- **Summary:** Full triage-and-fix pass against a 37-item UAT feedback sheet (stakeholder: Mohammed Saad Ansari) covering Admin Console screens A-02–A-20. Validated every item against running code before changing anything — did not assume the report was correct. **Confirmed and fixed (root cause, not UI-hiding), with tests:** #2 confusing `§13.1` glyph in the monitoring staleness note → plain "Section 13.1" text (`admin-ops.service.ts`); #6.2 onboarding form's City/Portal-password fields defaulted to literal `'Bengaluru'`/`'Password@01'` values instead of being empty with placeholder text → both now start empty (`onboarding/provider/page.tsx`); #11 Support Ticket Queue had no column sorting → click-to-sort added on ID/Requester/Priority/Status/Agent; #17 ticket internal notes silently overwrote instead of appending → `updateTicket` now appends a timestamped, actor-attributed entry to existing notes; #21 a resolved/closed ticket still accepted new notes and re-resolution with no visible reopen step → `updateTicket` now rejects further notes/status changes on a closed ticket unless the caller explicitly reopens it (status → non-terminal), frontend disables the note/resolve controls and adds an explicit "Reopen ticket" action; #25 SLA Monitoring showed every metric as BREACHED with zero real activity → root cause was `pct(n, Math.max(d, 1))` silently turning "no data" into a real 0%, defeating the function's own `d === 0` "no data" branch; fixed to pass the true denominator and render a neutral `NO_DATA` status instead of a false breach; #28 Knowledge Base's most-recently-selected article appeared twice (once as a compact list card, once again as a separate "expanded" card below) → selected article now renders expanded in place of its list card instead of being duplicated; #27 empty-body KB articles showed an ambiguous `—` → now says "No content yet for this article."; #30 Audit Search showed the raw Firebase-UID `actor` string → `searchAudit` now resolves it against the `ConsoleUser` directory and returns a human-readable `actorLabel` (email + role) when known; #32 Providers screen showed two contradictory status signals at once — the "Live registry" badge was unconditionally green regardless of onboarding progress, *and* a hospital's `address` was hardcoded to the literal string `'To be confirmed'` on creation rather than left unset → fixed both halves: `ProviderOnboardingService.createApplication` now leaves `address: null` (the frontend already only renders the address line when a real value exists) instead of writing a permanent placeholder string, and the Providers page badge now derives from `application.currentStage` (Live/Rejected/Onboarding-in-progress) instead of always reading "Live registry"; #18/#20 Support Ticket Detail's "Load case context" showed a confusing `No case found for entityRef Apollo Hospital, Whitefield` for PROVIDER tickets → root cause was `getTicketCaseContext` always searching `Case` by `entityRef` regardless of ticket type, but a PROVIDER ticket's `entityRef` is a facility name/id, not a case reference, by design — it now short-circuits for `requesterType === 'PROVIDER'` with a clear "linked to provider X, not a citizen case" note instead of attempting (and failing) a Case lookup; #35/#36 a console user created with no password (an optional invite field) had no way to ever be given one and could never log in, and "Resync access" crashed with Firebase's raw `auth/user-not-found` for such a user, surfacing as an opaque "Something went wrong" → added `ConsoleUserService.setConsolePassword` (provisions/updates the Firebase login and stamps claims, reusing the existing `provisionConsoleLogin` helper), a new `POST /v1/admin/console-users/:id/password` endpoint, and a "Set/Reset password" control in the Manage-staff panel; separately hardened `resyncConsoleClaims` to catch the missing-account case and throw a specific, actionable `BadRequestException` instead of an unhandled crash; #5 internal PRD reference codes (e.g. "G4", "FR-ADM-PRV-001") rendered on every single screen for every user via `TopBar`'s `ref_` prop → stopped rendering that line (prop kept for future dev-mode use); #1 profile name/email + sign-out existed only at the bottom of the sidebar → added a persistent identity/sign-out bar at the top of `ConsoleShell` (desktop breakpoint) without removing the sidebar's own control. **Confirmed not-a-bug / not reproducible:** #6.1 (stage gate correctly shows only the *current* stage as "Application submitted" — reviewed the actual stage-visual logic in `verify/page.tsx`, no dual-labelling found); #6.4/#8 (A-05 is a genuinely separate route/page from A-04, not a mis-route — `onboarding/provider/verify` has its own `screenId="A-05"` and is the only A-05 route in the app); #12/#13 (Support Ticket Queue's `requesterType`/`assignedAgent` are exactly what was submitted on ticket creation — `createTicket`/`updateTicket` do not silently alter either field; a "staff requester labelled CITIZEN" or "resolved ticket with no assigned agent" is a **data-entry outcome** of the create/update forms, not a code defect that mislabels tickets after the fact — no server-side inference or auto-classification logic exists to have gone wrong); #14/#15 (no "300 active tickets" stat exists anywhere in the current `dashboard/page.tsx` — nothing to reconcile against Support Ticket Queue's real count; A-19's provider-ticket count is a correctly-scoped `requesterType: PROVIDER` filter of the same real ticket table, not a separate/stale dataset); #16 (checked `lib/api.ts`'s `messageFromErrorBody` — 5xx bodies are already generic, 4xx bodies already extract Nest's `message` field only, no raw stack/internal text path found); #19 (Case-access justification is enforced both client- and server-side at ≥8 chars via `BadRequestException` in `getTicketCaseContext` — already mandatory, contrary to the report). **Logged as open product/design questions (no invented behavior):** #3 (Quick Links editing — deferred, no ticket needed beyond noting it's static by design), #4 (Citizen Onboarding Queue's review-panel placement/400-row claim — current code has no row limit and uses a side panel, not a bottom panel; this is a layout preference, not a bug, so left as-is pending explicit design direction), #6.3 (Org ID's purpose — used as the Firebase custom-claim / `OrgScopeGuard` key per DL-015, already explained in-code but not user-facing docs), #6.6/#9 (document attach-during-verification — real missing feature, too large for this pass, **TD-028**), #7 (Apollo-branded placeholder text — **DL-018**), #10/#22 (Go-Live vs. Access-Activate wording, Remote Session Assist flow — **DL-019**), #23/#24 (Issue Board transition legality / mandatory change-reason — **DL-020**), #26 (SLA org-scoping — **DL-021**), #29 (Configuration edit/plan-gating — **DL-022**), #31 (broader entity-label audit readability beyond the #30 actor fix — **DL-023**), #33/#34 (bed-total editability, provider category taxonomy — **DL-024**), #37 (role-permission matrix documentation — **DL-025**). Full item-by-item disposition table given to the requester in this session's final report (not persisted as a separate file per instruction not to create new docs).
- **Files Changed:** `apps/api/src/admin/admin-ops.service.ts` (staleness note text; ticket append-only notes + closed-ticket reopen guard; SLA zero-volume→`NO_DATA` fix; audit actor-label resolution; PROVIDER-ticket case-context short-circuit); `apps/api/src/admin/admin-ops.service.unit.spec.ts` (regression tests: append-only notes, closed-ticket guard, PROVIDER entityRef short-circuit, SLA zero-volume, audit actor-label); `apps/api/src/admin/provider-onboarding.service.ts` (`address: null` instead of hardcoded `'To be confirmed'`); `apps/api/src/admin/console-user.service.ts` (new `setConsolePassword`; `resyncConsoleClaims` actionable error on missing account); `apps/api/src/admin/console-user.service.unit.spec.ts` (regression tests: `setConsolePassword` success + short-password rejection, `resyncConsoleClaims` actionable error); `apps/api/src/admin/admin.controller.ts` (new `POST console-users/:id/password`); `apps/api/src/admin/admin.controller.unit.spec.ts` (covers the new endpoint); `apps/admin-console/src/app/(console)/onboarding/provider/page.tsx` (removed literal default form values); `apps/admin-console/src/app/(console)/support/tickets/page.tsx` (column sorting); `apps/admin-console/src/app/(console)/support/tickets/detail/page.tsx` (append-note UX, closed-ticket guard/reopen control); `apps/admin-console/src/app/(console)/issues/sla/page.tsx` (render `n/a` for null compliance); `apps/admin-console/src/app/(console)/knowledge-base/page.tsx` (selected-article-in-place, empty-body copy); `apps/admin-console/src/app/(console)/governance/page.tsx` (actorLabel rendering); `apps/admin-console/src/app/(console)/providers/page.tsx` (status-consistent live/onboarding badge); `apps/admin-console/src/app/(console)/users/page.tsx` (new Set/Reset-password control calling the new endpoint); `apps/admin-console/src/components/shell/TopBar.tsx` (stopped rendering internal `ref_` line); `apps/admin-console/src/components/shell/ConsoleShell.tsx` (top-of-page profile/sign-out bar); `apps/admin-console/src/lib/api.ts` (`AuditRow.actorLabel`, `SlaRow.compliancePercent`/`variant` nullable/`neutral`, `users.setPassword`); `IMPLEMENTATION_MASTER_PLAN.md` (this update, TD-028–030, DL-018–025).
- **Features Completed:** 15 of 37 UAT items confirmed as real bugs and fixed at the root cause, with new/updated Jest regression tests for every server-side logic change (ticket notes/reopen guard, PROVIDER case-context short-circuit, SLA zero-volume, console-user password set/resync); several UI-only fixes (sorting, badge/copy consistency, missing control wiring) rely on existing/adjacent suites plus manual `tsc`/`eslint`/`next build` verification rather than new dedicated component tests, since the frontend Jest suite in this app is smoke-level (12 tests total, no per-page component tests) — flagged, not silently left uncovered. 7 items confirmed not reproducible or already-correct against current code (documented why, not just asserted). 15 items logged as genuine open product/design questions (TD-028–030, DL-018–025) rather than guessed at.
- **Verification:** `apps/api` — `npm run lint` clean, `npm run typecheck` clean, `npx jest` (non-DB suites) 226/226 passed + 7 DB-integration suites cleanly skip via `SKIP_DB_INTEGRATION` (pre-existing, TD-001 — no local Postgres in this session), `npm run build` produces `dist/main.js` (re-verified per ISSUE-006's caution — also had to clear a stale `tsconfig.build.tsbuildinfo` this session to get a real rebuild). `apps/admin-console` — `eslint .` clean, `tsc --noEmit` clean, `npx jest` 12/12 passed, `npm run build` (Next.js production build) succeeds, all 24 routes prerender including every touched page.
- **Current Blockers:** None. TD-028 (document upload/attach for provider verification) is the one confirmed real gap intentionally deferred as too large for this pass — needs an actual object-storage-backed upload endpoint before it can be built.
- **Next Recommended Task:** Build TD-028 (provider-verification document upload) if prioritized next. Otherwise, work through the DL-018–025 open questions with the product owner and convert whichever get resolved into follow-up implementation sessions. Consider adding page-level component tests for admin-console (currently smoke-level only) as a separate follow-up, independent of this UAT pass.
